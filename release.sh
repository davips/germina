...
source /home/davi/.cache/pypoetry/virtualenvs/germina-_FW2ScvW-py3.10/bin/activate
pip install .
#################################################################################
#################################################################################
rm docs -rf
poetry run black -l200 src/ tests/
poetry run pdoc --html --force germina -o docs
mv docs/germina/* docs/
rm docs/germina -rf
git add docs
