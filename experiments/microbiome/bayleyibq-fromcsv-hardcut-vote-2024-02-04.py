...
def cut(df: DataFrame, yr, low, high, low2=0, high2=999):
    if low == "longitudinal":
        df_t3 = read_csv(f"data/workshop111223.csv", index_col="id_estudo")[["bayley_8_t3"]]
        df = df.join(df_t3)
        df = df.dropna(axis="rows", how="any")
        yr2 = df["bayley_8_t3"]
        hiidx = df.index[yr2 > yr + high]
        loidx = df.index[yr2 < yr - high]
    elif high is None:
...
M, delta, trees, sp, longitudinal = 3, 7.5, 1024, int(argv[1]), bool(argv[2])
...
X0, y0, _, _ = cut(df, yr, None, None)
X, y, s0, s1 = [DataFrame()] * M, [DataFrame()] * M, [DataFrame()] * M, [DataFrame()] * M
if longitudinal:
    M = delta = 1
    X[0], y[0], s0[0], s1[0] = cut(df, yr, "longitudinal", None)
else:
    start = 100 - delta / 2
    step = delta / (M - 1)
    for i in ap[0, 1, ..., M - 1]:
        # print(start + i * step - delta / 2, start + i * step + delta / 2)
        X[i], y[i], s0[i], s1[i] = cut(df, yr, start + i * step - delta / 2, start + i * step + delta / 2)
# exit()
loo = LeaveOneOut()
