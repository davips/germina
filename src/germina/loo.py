...
def imputation(Xy_tr, baby, alg, n_estimators, seed, jobs):
    print("\timputing", end="", flush=True)
    imp = imputer(alg, n_estimators, seed, jobs)
...
def fselection(Xy_tr, baby, alg, n_estimators, forward, k_features, k_folds, seed, jobs):
    print("\tselecting", end="", flush=True)
    sel = selector(forward, alg, n_estimators, k_features, k_folds, seed, jobs)
...
def predictors(alg, n_estimators, seed, jobs):
    if alg == "rf":
        ...
        algclass_c = LGBMc(n_estimators=n_estimators, random_state=seed, n_jobs=jobs)
        algclass_r = LGBMr(n_estimators=n_estimators, random_state=seed, n_jobs=jobs)
    elif alg == "et":
...
def train_c(pairs_X_tr, pairs_y_tr_c, alg, n_estimators, seed, jobs):
    print("\ttrainingC", end="", flush=True)
    alg_c = predictors(alg, n_estimators, seed, jobs)[0]
    alg_c.fit(pairs_X_tr, pairs_y_tr_c)
    return alg_c


def train_r(pairs_X_tr, pairs_y_tr_r, alg, n_estimators, seed, jobs):
    print("\ttrainingR", end="", flush=True)
    alg_r = predictors(alg, n_estimators, seed, jobs)[1]
...
def loo(df: DataFrame, permutation: int, pairwise: str, threshold: float, rejection_threshold: float,
        alg, n_estimators,
        n_estimators_fsel_imput, forward_fsel, k_features_fsel, k_folds_fsel,
    ...
    """
    if pairwise not in {"concatenation", "difference", "concatenation%", "difference%"}:  # TODO: %
    ...
    # LOO
    d = hdict(df=df, alg=alg, n_estimators=n_estimators_fsel_imput, forward=forward_fsel, k_features=k_features_fsel, k_folds=k_folds_fsel, seed=seed, _jobs_=jobs)
    hits_c, hits_r = {0: 0, 1: 0}, {0: 0, 1: 0}
    tot_c, tot_r = {0: 0, 1: 0}, {0: 0, 1: 0}
    y = df.iloc[-1].to_numpy()
    y_c, y_r, z_lst_c, z_lst_r = [], [], [], []
    ansi = d.hosh.ansi
    ...
    for c, (id, per, idx) in enumerate((Scheduler(db, timeout=60) << tasks) if sched else tasks):
        if not sched:
            print(f"\r Permutation: {permutation:8}\t\t{ansi} baby {idx}: {100 * c / len(df):8.5f}%             ", end="", flush=True)
        ...
        # train
        d.apply(train_c, pairs_X_tr, pairs_y_tr_c, jobs=_._jobs_, out="result_train_c")
        d = ch(d, storages)
        if not sched:
            print(f"\r Permutation: {permutation:8}\t\t{ansi} baby {idx}: {100 * c / len(df):8.5f}%             ", end="", flush=True)
        d.apply(train_r, pairs_X_tr, pairs_y_tr_r, jobs=_._jobs_, out="result_train_r")
        d = ch(d, storages)
    ...
    return d, bacc_c, bacc_r, r2_c, r2_r, hits_c, hits_r, tot_c, tot_r, rj_c, rj_r
